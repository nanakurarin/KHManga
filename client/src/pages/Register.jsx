...
import { syncUserWithBackend, checkUsernameAvailability } from '../services/backend';
 ...
 * Creates the user and sets their displayName.
 * 
...
function Register() {
  ...
  /**
   * Form Submission Handler following exact required flow:
   * 1. Validate form inputs
   * 2. Check username availability on Backend (MySQL)
   * 3. If available: Create Firebase Auth account
   * 4. Sync user data to Backend (MySQL) with Rollback handling
   * 5. Navigate Home
   */
  const handleSubmit = async (e) => {
    ...
    try {
      // Step 2: Check username availability on MySQL backend BEFORE Firebase account creation
      const usernameCheck = await checkUsernameAvailability(username.trim());

      if (!usernameCheck.available) {
        // If username exists in MySQL: show error & stop (do NOT create Firebase user)
      ...
      // Step 4: Sync user data to MySQL backend
      try {
        await syncUserWithBackend({
          firebaseUid: registeredUser.uid,
          email: registeredUser.email,
          username: registeredUser.displayName || username.trim(),
        });
      } catch (syncErr) {
        // Rollback: If MySQL insertion fails after Firebase creation, delete the Firebase user
        console.error('MySQL sync failed. Rolling back Firebase account...', syncErr);
        try {
          await deleteUser(registeredUser);
        } catch (deleteErr) {
          console.error('Failed to rollback Firebase user creation:', deleteErr);
        }
        throw new Error(syncErr.message || 'Failed to complete account registration in database.');
      }

      // Step 5: Navigate Home after successful sync
      navigate('/');
    } catch (err) {
